...
const ScoreBoard = () => {
  ...
  useEffect(() => {
    const fetchMatchData = async () => {
      try {
        ...
        setTotalSet(data.number_of_sets || 2);
        setWinningPoints(data.winning_points || 3);
        set.match_time &&
          setTimer({
            minutes: parseInt(set.match_time.split(":")[0]),
            seconds: parseInt(set.match_time.split(":")[1]),
            running: false,
          });
        ...
        if (data.match_type === "single") {
          setPlayers({
            teamA: {
              ...
              score: set.match_score_teamA || 0,
            },
            teamB: {
              player1Name: data.player2,
              score: set.match_score_teamB || 0,
            },
          });
        } else {
          setPlayers({
  ...
  const saveMatchData = async (winner = "") => {
    try {
      ...
      let status =
        winner.length > 0 && matchLog.length >= totalSet ? "completed" : "";
  ...
  const checkWinner = () => {
    ...
    console.log("team A", teamAScore, "team B", teamBScore, winningPoints);
    ...
    <main className="admin-wrapper d-flex w-100 flex-wrap bg-EEEEEE">
      ...
      <section className="right-content-wrapper overflow-auto custom-scroll1">
        ...
        <div className="container mt-4 text-center">
          <h3>
            Badminton Scoreboard ({matchType})-({currentSet})
          </h3>
              ...
              {Object.keys(players.teamA).map((key, index) =>
                key.includes("player") ? (
                  <div className="player-name" key={index}>
                    ...
                    {status != "completed" &&
                      currentServer.team === "teamA" &&
                      currentServer.player === key && (
                      ...
                      currentServer.player === key && (
                        <>
                          <img
                            ...
                            alt="shuttle image"
                            className="shuttle-indicator"
                          />
                          {currentServer.player === "player1Name" ? (
                            <img
                              src="/images/arrow.png"
                              alt="arrow image"
                              className="left-top-arrow-image"
                            />
                          ) : (
                            <img
                              src="/images/arrow.png"
                              alt="arrow image"
                              className="left-bottom-arrow-image"
                            />
                          )}
                        </>
                      )}
                        ...
                        <>
                          ...
                          {currentServer.player === "player1Name" ? (
                            <img
                              src="/images/arrow.png"
                              alt="arrow image"
                              className="right-top-arrow-image"
                            />
                          ) : (
                            <img
                              src="/images/arrow.png"
                              alt="arrow image"
                              className="right-bottom-arrow-image"
                            />
                          )}
                        </>
                      )}
                  ...
                  <table className="table table-bordered text-center">
                    <thead>
                      <tr>
                        ...
                        <th></th>
                        {set.log.length === 0 ||
                        set.log[set.log.length - 1]?.rally <= 21
